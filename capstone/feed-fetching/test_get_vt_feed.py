...
@patch('requests.get', return_value=MockResponse(status_code=200, content=b'vt-feed'))
@patch('bz2.open', return_value=io.BytesIO(b'{ "attributes": { "attr1": "val1", "attr2": "val2" } }'))
def test_get_feed(bz2_open, requests_get, os_environ):
    import get_vt_feed
    result = list(get_vt_feed.get_feed(0))

    os.environ.get.assert_called_once_with('VTKEY')
...
def test_prepare_doc():
    ...
    feed = {field: field for field in get_vt_feed.fields_allowed}
